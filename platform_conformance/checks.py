"""Offline conformance checks for Paqto public APIs and local capabilities."""
...
from platform_conformance.models import CapabilityUnavailable, Check

CERTIFICATES = Path(__file__).parent / "certificates"
...
class JsonEnvelopeSerializer(Serializer):
    """Small deterministic serializer used only by conformance checks."""

    @property
    def protocol_id(self) -> str:
        return "paqto.conformance.json.v1"
...
def _config(**overrides: Any) -> PaqtoConfig:
    values: dict[str, Any] = {
        ...
        "serializer_id": "paqto.conformance.json.v1",
    }
