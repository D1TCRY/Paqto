...
For development, install the optional dependencies and run the checks manually:
...
These commands are not attached to `git push`; pushing only publishes the
commits already present in the local branch.
...
[platform testing](docs/platform-testing.md) for report semantics and
real-device procedures.
...
See [Create two LAN nodes](docs/getting-started.md#create-two-lan-nodes) for a
